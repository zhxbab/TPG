...
class Mode(Util):
    ...
    def Protect_mode_code(self):
        ...
        if self.intel:
            for i in range(0,self.threads):
                if i == 0x0:
                    self.instr_manager.Set_instr(60,0)
                    self.simcmd.Add_sim_cmd("at $y%d >= %d set register EAX to 0x000000%02x"%(i,self.instr_manager.Get_instr(i),i*0x20),i)
                else:
                    self.instr_manager.Set_instr(56,i)
                    self.simcmd.Add_sim_cmd("at $y%d >= %d set register EAX to 0x000000%02x"%(i,self.instr_manager.Get_instr(i),i*0x20),i)
        ...
        for i in range(0,self.threads):
            if i == 0x0:
                self.instr_manager.Set_instr(62,0)
            else:
                self.instr_manager.Set_instr(58,i)