...
#        self.fail_ic_new_name = self.fail_ic_new_name.split("/")[-1]
    ...
    def Tune_clk_and_feature(self):
        for clc in self.clk_list_choice:
            ...
            runpclmsi_cmd = "%s +device:%d +avpl:%s +log_name:%s +clkRatio:%s %s"%(self.runpclmsi,self.device,self.temp_list,self.base_name,clc,self.bustool_cmd)
            ...
            if len(self.feature_list_choice) == 0:
                ...
                for feature in self.feature_list_choice:
                    runpclmsi_cmd = "%s +device:%d +avpl:%s +log_name:%s +clkRatio:%s +flip_msr_bit:\"%s\" %s"\
